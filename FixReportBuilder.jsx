  ...
  ({
    ...
    const addInValue = useCallback(
      (index, rawInput) => {
        ...
        // Support typing or pasting several values at once, separated by
        // commas — e.g. "2134760004, 2766554432" becomes two separate
        // values instead of one literal string containing a comma.
        const incomingValues = rawInput
          .split(",")
          .map((v) => v.trim())
          .filter((v) => v.length > 0);

        if (incomingValues.length === 0) return;

        // De-duplicate against existing values and within the pasted batch
        // itself, so typing the same number twice doesn't create two chips.
        const seen = new Set(currentValues);
        const uniqueNewValues = [];
        for (const value of incomingValues) {
          if (!seen.has(value)) {
            seen.add(value);
            uniqueNewValues.push(value);
          }
        }
        ...
        newConditions[index] = {
          ...
          condition: {
            ...newConditions[index].condition,
            value: [...currentValues, ...uniqueNewValues],
            dataType: newConditions[index].condition.dataType,
          },
        };
        updateConditions(newConditions);
              ...
              <Box sx={{ mt: 1.5, pl: 1 }}>
                ...
                <Box sx={{ display: "flex", gap: 1, alignItems: "flex-start" }}>
                  <TextField
                    size="small"
                    label="Add value"
                    placeholder="e.g. 2134760004, 2766554432"
                    ...
                    onPaste={(e) => {
                      ...
                      if (pasted.includes(",")) {
                        // A multi-value paste — commit it immediately
                        // instead of waiting for Enter/Add.
                        e.preventDefault();
                        addInValue(index, pasted);
                      }
                    }}
                    sx={{ flex: 1 }}
                  />
